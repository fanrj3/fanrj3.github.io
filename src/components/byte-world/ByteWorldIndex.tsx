'use client';

import Link from 'next/link';
import { ArrowUpRight, BookOpen, Clock3, Code2, FlaskConical } from 'lucide-react';
import { useLocaleStore } from '@/lib/stores/localeStore';
import type { ByteWorldEntry } from '@/lib/byteWorld';

const copy = {
  en: {
    eyebrow: 'IMPLEMENTATION JOURNAL',
    title: 'Byte World',
    description: 'Small systems built from first principles. Notes on the mathematics, implementation choices, mistakes, and experiments that made each idea click.',
    count: 'notes',
    read: 'min read',
    open: 'Read note',
    all: 'All notes',
  },
  zh: {
    eyebrow: '从原理到实现',
    title: 'Byte World',
    description: '从第一性原理实现小型系统，记录数学推导、工程取舍、踩坑过程与验证实验，让理解真正落到代码里。',
    count: '篇笔记',
    read: '分钟',
    open: '阅读笔记',
    all: '全部笔记',
  },
};

const statusLabels: Record<string, Record<string, string>> = {
  en: { complete: 'Complete', 'in-progress': 'In progress' },
  zh: { complete: '已完成', 'in-progress': '持续更新' },
};

export default function ByteWorldIndex({ entries }: { entries: ByteWorldEntry[] }) {
  const locale = useLocaleStore((state) => state.locale);
  const language = locale.startsWith('zh') ? 'zh' : 'en';
  const text = copy[language];
  const featured = entries.find((entry) => entry.featured) ?? entries[0];
  const rest = entries.filter((entry) => entry.slug !== featured?.slug);

  return (
    <div className="byte-index">
      <header className="byte-index-hero">
        <div>
          <p className="byte-eyebrow"><Code2 aria-hidden="true" /> {text.eyebrow}</p>
          <h1>{text.title}</h1>
          <p>{text.description}</p>
        </div>
        <div className="byte-index-count" aria-label={`${entries.length} ${text.count}`}>
          <strong>{String(entries.length).padStart(2, '0')}</strong>
          <span>{text.count}</span>
        </div>
      </header>

      {featured && (
        <Link href={`/byte-world/${featured.slug}`} className="byte-featured">
          <div className="byte-featured-mark" aria-hidden="true">
            <FlaskConical />
            <span>01</span>
          </div>
          <div className="byte-featured-body">
            <div className="byte-card-meta">
              <span>{featured.series || 'Build log'}</span>
              <span>{featured.date}</span>
              <span><Clock3 aria-hidden="true" /> {featured.readingTime} {text.read}</span>
            </div>
            <h2>{featured.title}</h2>
            <p>{featured.description}</p>
            <div className="byte-tag-row">
              {featured.tags?.map((tag) => <span key={tag}>{tag}</span>)}
            </div>
          </div>
          <span className="byte-open-link">{text.open} <ArrowUpRight aria-hidden="true" /></span>
        </Link>
      )}

      {rest.length > 0 && (
        <section className="byte-archive" aria-labelledby="byte-archive-title">
          <div className="byte-section-heading">
            <BookOpen aria-hidden="true" />
            <h2 id="byte-archive-title">{text.all}</h2>
          </div>
          <div className="byte-note-list">
            {rest.map((entry, index) => (
              <Link href={`/byte-world/${entry.slug}`} className="byte-note-row" key={entry.slug}>
                <span className="byte-note-number">{String(index + 2).padStart(2, '0')}</span>
                <div>
                  <div className="byte-card-meta">
                    <span>{entry.series || 'Build log'}</span>
                    <span>{entry.date}</span>
                  </div>
                  <h3>{entry.title}</h3>
                  <p>{entry.description}</p>
                </div>
                <div className="byte-note-aside">
                  {entry.status && <span>{statusLabels[language][entry.status] || entry.status}</span>}
                  <ArrowUpRight aria-hidden="true" />
                </div>
              </Link>
            ))}
          </div>
        </section>
      )}
    </div>
  );
}
